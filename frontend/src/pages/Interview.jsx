  ...
  return (
    <main className="min-h-[calc(100vh-144px)] flex flex-col md:flex-row max-w-7xl mx-auto px-6 py-8 gap-8">
      ...
      <div className="flex-none w-full md:w-80 flex flex-col gap-6">
        <section>
          <p className="text-on-surface-variant font-[Inter] text-xs uppercase tracking-[0.2em] mb-2">Current Session</p>
        ...
        <div className="bg-surface-container-low rounded-xl p-6 ghost-border">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-xs text-on-surface-variant leading-relaxed italic">
              "I'll be evaluating your fit for the {roleTitle || 'role'} at {companyName || 'the company'} today."
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface-container-low rounded-xl p-4 ghost-border text-center">
          ...
          <p className="text-2xl font-[Manrope] font-bold text-primary tabular-nums">{formatTime(elapsed)}</p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-on-surface-variant leading-normal">
                Speak clearly and use the STAR method. The AI is grounded in real {companyName || 'company'} data.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col gap-6 min-h-[500px]">
        ...
        <div className="glass-hud p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-outline-variant/20">
          <div className="flex items-center gap-6">
            ...
            >
              <div className={`w-16 h-16 rounded-full flex items-center justify-center border border-outline-variant/30 transition-colors ${muted ? 'bg-error-container' : 'bg-surface-container-highest group-hover:bg-surface-bright'}`}>
                <span className="material-symbols-outlined text-2xl text-on-surface">{muted ? 'mic_off' : 'mic'}</span>
              </div>
          ...
          <button
            ...
            disabled={ending}
            className="flex-1 md:flex-none px-8 py-4 bg-error-container text-error rounded-xl font-[Manrope] font-bold flex items-center justify-center gap-3 transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-60"
          >
            {ending ? (
              <>
                <span className="animate-spin h-5 w-5 border-2 border-error border-t-transparent rounded-full"></span>
                Generating Report...
              </>
            ) : (
              <>
