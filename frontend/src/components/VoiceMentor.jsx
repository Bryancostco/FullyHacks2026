...
export default function VoiceMentor({ sessionId, autoStart = false, onStatusChange, onStop, onTurnAdded, stopRef }) {
      ...
      dc.onopen = () => {
        ...
        dc.send(JSON.stringify({
          ...
          item: {
            type: 'message',
            role: 'user',
            content: [{ type: 'input_text', text: 'Hello, I am ready to begin the interview.' }],
          },
        }));
      ...
      dc.onmessage = (e) => {
        try {
          ...
          if (serverEvent.type === 'conversation.item.input_audio_transcription.completed') {
            const userText = serverEvent.transcript?.trim();
            if (userText) {
              const turn = { role: 'user', text: userText };
              turnsRef.current.push(turn);
              onTurnAdded?.(turn);
            }
          }
